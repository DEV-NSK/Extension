...
# #         # Get activities for last 7 days for chart data
...
# #         for activity in today_activities[:20]:  # Only get recent 20 for display
# #             # Count domains
# #             if activity.domain:
# #                 domains_visited[activity.domain] = domains_visited.get(activity.domain, 0) + 1
...
# #         page_visits = sum(1 for a in today_activities if a.activity_type == 'page_visit')
        
# #         # Calculate total interactions (all activities)
# #         total_interactions = len(today_activities)
        
# #         # Calculate total time spent
# #         total_time_spent = sum(a.duration_seconds for a in today_activities)
...
# #             "total_pages": page_visits,  # For backward compatibility
# #             "page_visits": page_visits,  # Added for clarity
# #             "total_interactions": total_interactions,
# #             "interactions": total_interactions,  # Added for clarity
...
# #         logger.info(f"Summary for user {user_id}: {page_visits} pages, {total_interactions} interactions")
...
#         # Log for debugging
#         logger.info(f"Getting summary for user: {user_id}, today: {today}")
...
#         logger.info(f"Found {len(today_activities)} activities today for user {user_id}")
        
#         # Get ALL activities for this user (not just today) for debugging
#         all_activities = BrowserActivity.query.filter_by(user_id=user_id).all()
#         logger.info(f"Total activities for user {user_id}: {len(all_activities)}")
...
#         # If we have a daily session, use its values for totals
#         if daily_session:
#             page_visits = max(page_visits, daily_session.total_pages_visited or 0)
#             total_interactions = max(total_interactions, daily_session.total_interactions or 0)
...
#             "avg_time_per_interaction": avg_time_per_interaction,
#             "unique_domains": len(domains_visited),
#             "active_days": max(active_days, 1),  # At least 1 day
#             "domains_visited": domains_visited,
#             "activity_breakdown": activity_types_count,
#             "recent_activities": recent_activities,
#             "daily_activity": daily_activity,
